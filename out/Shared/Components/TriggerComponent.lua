...
function TriggerComponent.new(part: BasePart, touchType: string?)
	...
	local self = setmetatable({}, TriggerComponent)

	self.part = part
	self.touchType = touchType or TriggerComponent.TouchType.Spatial
	...
	if self.touchType == TriggerComponent.TouchType.Spatial then
		table.insert(activeComponents, self)
		if not runServiceConnection then
			runServiceConnection = RunService.Heartbeat:Connect(TriggerComponent.updateAll)
		end

	elseif self.touchType == TriggerComponent.TouchType.Physics then
		self:initPhysics()
	end
...
function TriggerComponent:initPhysics()
...
function TriggerComponent.updateAll()
	for i = #activeComponents, 1, -1 do
		activeComponents[i]:checkProximity()
	end
end

function TriggerComponent:checkProximity()
	if self._isDestroyed then return end
...
function TriggerComponent:clearAllCurrent()
	local playersToEnd = {}
...
function TriggerComponent:Destroy()
	if self._isDestroyed then return end
