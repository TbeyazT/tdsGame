...
local function Load()
	...
	local ServerFolders = ServerScriptService.Server.Services:GetChildren()
	local modulesToInit = {}
	...
	for _, Folder in ipairs(ServerFolders) do
		if Folder:IsA("Folder") then
			...
			for _, child in ipairs(Folder:GetChildren()) do
				if child:IsA("ModuleScript") and string.find(child.Name, "Server") then
					foundModule = child
					break
				end
			end
			...
			if foundModule then
				...
				warn(PREFIX .. " ⚠️ No Server module found in folder: " .. Folder:GetFullName())
			end
		end
	end
