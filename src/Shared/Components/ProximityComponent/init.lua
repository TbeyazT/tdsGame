...
local AnimationComponent = require(Shared.Components.AnimationComponent)
...
function ProximityComponent.new(Data): Proximity
	...
	if typeof(self.Root) == "Vector3" then
		self.AdorneeInstance = Instance.new("Attachment")
		self.AdorneeInstance.WorldPosition = self.Root
		self.AdorneeInstance.Name = "ProximityAttachment"
		self.AdorneeInstance.Parent = workspace.Terrain
		self._ownsAdornee = true
	elseif typeof(self.Root) == "Instance" then
		if self.Root:IsA("Model") then
			self.AdorneeInstance = self.Root.PrimaryPart or self.Root:FindFirstChildWhichIsA("BasePart", true)
		else
			self.AdorneeInstance = self.Root
		end
	end
...
function ProximityComponent:OpenUI()
	...
	self.UI.Adornee = self.AdorneeInstance
...
function ProximityComponent:Destroy()
	...
	if self._ownsAdornee and self.AdorneeInstance then
		self.AdorneeInstance:Destroy()
	end
	
	self.Root = nil
	self.Target = nil
	self.AdorneeInstance = nil
